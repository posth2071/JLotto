package com.JLotto.JLotto.AsyncTask;
...
public class TestClass {
    ...
    public class TaskClass extends AsyncTask<String, Void, LottoParsingInfo> {
        private final String STORE_URL = "https://dhlottery.co.kr/store.do?method=topStore&pageGubun=L645"; //
        private final String LAST_RESULT_URL = "https://dhlottery.co.kr/gameResult.do?method=byWin&wiselog=C_A_1_2"; //
        private final String SELECT_RESULT_URL = "https://dhlottery.co.kr/gameResult.do?method=byWin&drwNo=%s"; //
        ...
        protected LottoParsingInfo doInBackground(String... num) {
            ...
            try {
                if (num[0] == "") {           //""인경우 제일최신회차 파싱
                    doc1 = Jsoup.connect(STORE_URL).get();  //1등 당첨장소 파싱
                    ...
                    for (Element ele : ele2) {
                        Elements ele3 = ele.select("td");   // 5개 항목 (번호, 상호명, 구분, 소재지, 위치보기)

                        if(ele3.size() > 1) {   // 상호명, 소재지만 구하기 (인덱스 1,3)
                    ...
                    //마지막회차 당첨번호 파싱
                    doc1 = Jsoup.connect(LAST_RESULT_URL).get();
                } else {
                    // num[0]==""이 아닌경우 (회차입력한 경우)
                    // 검색회차 파싱
                    doc1 = Jsoup.connect(String.format(SELECT_RESULT_URL, num[0])).get();
                }
                // 회차정보 파싱
                Elements contents1 = doc1.select("div.win_result h4");

                //해당회차 번호저장
                resultParsingInfo.setTurn(Integer.parseInt(contents1.text().replace("회 당첨결과","")));
